...
def loss_temporal_rank(x: torch.Tensor, gt: torch.Tensor):
    B, N, K, _ = x.shape
    x_depth1 = x[..., 2].permute(0, 2, 1).unsqueeze(-1).repeat_interleave(N, -1)
    x_depth2 = x[..., 2].permute(0, 2, 1).unsqueeze(-2).repeat_interleave(N, -2)
    gt_depth1 = gt[..., 2].permute(0, 2, 1).unsqueeze(-1).repeat_interleave(N, -1)
    gt_depth2 = gt[..., 2].permute(0, 2, 1).unsqueeze(-2).repeat_interleave(N, -2)
    C = (x_depth1 - x_depth2)
    D = (gt_depth1 < gt_depth2)
    rankloss = torch.maximum(C, torch.zeros_like(gt_depth1)) * D
    return torch.mean(rankloss)