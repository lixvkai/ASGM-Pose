...
class CrossScan_bs_ft(torch.autograd.Function):
    ...
    def backward(ctx, ys: torch.Tensor):
        ...
        L = H * W
        ys = ys[:, 0:2] + ys[:, 2:4].view(B, 2, -1, L)
        y = ys[:, 0].view(B, -1, H, W).flip(dims=[-1]).contiguous().view(B, -1, L) + ys[:, 1].view(B, -1, W,
                                                                                                   H).transpose(dim0=2,
                                                                                                                dim1=3).contiguous().view(
            B, -1, L)
        return y.view(B, -1, H, W)
...
class CrossMerge_bs_ft(torch.autograd.Function):
    ...
    def forward(ctx, ys: torch.Tensor):
        ...
        ys = ys.view(B, K, D, -1)
        ys = ys[:, 0:2] + ys[:, 2:4].view(B, 2, D, -1)
        y = ys[:, 0].view(B, -1, H, W).flip(dims=[-1]).contiguous().view(B, D, -1) + ys[:, 1].view(B, -1, W,
                                                                                                   H).transpose(dim0=2,
                                                                                                                dim1=3).contiguous().view(
            B, D, -1)
        return y
...
class CrossScan_bs_bt(torch.autograd.Function):
    ...
    def backward(ctx, ys: torch.Tensor):
        ...
        L = H * W
        ys = ys[:, 0:2] + ys[:, 2:4].view(B, 2, -1, L)
        y = ys[:, 0].view(B, -1, H, W).flip(dims=[-1]).contiguous().view(B, -1, L) + ys[:, 1].view(B, -1, W, H).flip(
            dims=[-1]).transpose(dim0=2, dim1=3).contiguous().view(B, -1, L)
        return y.view(B, -1, H, W)
...
class CrossMerge_bs_bt(torch.autograd.Function):
    ...
    def forward(ctx, ys: torch.Tensor):
        ...
        ys = ys.view(B, K, D, -1)
        ys = ys[:, 0:2] + ys[:, 2:4].view(B, 2, D, -1)
        y = ys[:, 0].view(B, -1, H, W).flip(dims=[-1]).contiguous().view(B, D, -1) + ys[:, 1].view(B, -1, W, H).flip(
            dims=[-1]).transpose(dim0=2, dim1=3).contiguous().view(B, D, -1)
        return y
